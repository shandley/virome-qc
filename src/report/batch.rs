...
<div class="chart-row" id="row-contam">
  <div class="chart-panel" id="panel-rrna"><h3>rRNA Fraction</h3><canvas id="ch-rrna"></canvas></div>
  <div class="chart-panel" id="panel-host"><h3>Host Fraction</h3><canvas id="ch-host"></canvas></div>
</div>
...
(function() {{
  ...
  // Hide panels where all values are zero
  const allZero = (arr) => arr.every(v => v === 0 || v < 0.0001);
  if (allZero(samples.map(s => s.rrna_rate))) document.getElementById('panel-rrna').style.display = 'none';
  if (allZero(samples.map(s => s.host_rate))) document.getElementById('panel-host').style.display = 'none';
  // Hide entire row if both panels hidden
  const row = document.getElementById('row-contam');
  if (row && !row.querySelector('.chart-panel[style=""],.chart-panel:not([style])')) {{
    // Check if any child is visible
    const visible = Array.from(row.children).some(c => c.style.display !== 'none');
